...
export default function Services() {
  return (
    <section className="relative flex w-full flex-wrap justify-center gap-20 px-20 py-20 max-sm:px-0 " id="services">
      <span className="bg_blend absolute left-0 top-0 size-full"></span>
      {services.map((card, index) => (
        <div
          className="relative flex h-[20rem] w-[24rem] max-lg:w-[30rem] max-lg:h-[24rem] max-sm:w-[24rem] max-sm:h-[20rem] flex-col items-center justify-evenly overflow-hidden rounded-2xl bg-black p-2 px-[2.5rem] text-center shadow-lg"
          key={index}
        >
          <div className="">
